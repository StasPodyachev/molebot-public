...
function encodeMintCredits(to: string, amount: bigint): string {
 ...
 */
export function hasPendingTopUp(): TopUpRequest {
  if (!pendingRequest) {
...
export async function requestTopUp(
  agentWallet: `0x${string}` = AGENT_WALLET,
  topUpAmount: bigint = BigInt('100000000000000000000'), // 100 токенов
): Promise<{ ok: boolean; txId: string | null; error?: string }> {
 ...
 */
export function resetPendingTopUp(): void {
  pendingRequest = null;
}
